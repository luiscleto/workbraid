...
Prerequisite: Architecture Reviews 1 completion `94787a8b05fdc502aedd07aae34b5d016b7106a3`, implementation `892f3254ad13b0fdabe02202e0070ac153f07bb8`.

This document defines the approved implementation contract for Reconciliation and the ordinary detail-link reassignment extension. It supplements, rather than rewrites, the completed Architecture, Change Sets, Reviews, and Agent Access records. The portable Architecture format remains v2. Rich-diagram Phase 3 remains deferred.
...
## 2. Ordinary detail-link reassignment
...
### 2.2 Minimal concrete pending representation
...
### 2.3 Closed operational schema evolution

`changes.yaml` uses **version 2**, retaining `format: workbraid-change-state` and every existing item spelling, with seven required sequences:
...
## 3. Three-way semantic comparison
...
### 3.2 Relationship consequences and order
...
### 3.3 Structural conflicts and competing children
...
### 3.4 External lifecycle and unsupported choices
...
## 4. Fidelity and reconstruction relative to A
...
6. Submit those facts to the one `ConstructCandidate(A, facts)`, load/validate the resulting complete tree through the existing format-v2 path, and check that its semantic facts equal the explicit resolved result. Constructor normalization must not silently swallow a conflict choice.
...
## 5. Preview, resolutions, and atomic apply

### 5.1 Prepare reconciliation
...
### 5.2 Closed request-local resolution vocabulary
...
## 7. CLI, MCP, skill and local protocol
...
| `change_set_state_mismatch` (new) | Exact active state object changed, including a retry with old S after successful Apply. Return truthful current Change Set context; inspect it before preparing again. Never replay blindly or infer the cause of an arbitrary later state. |
...
| `reconciliation_unresolved` (new) | Required scalar choices or displaced-child/structural assignments remain incomplete. |
| `target_not_found` / `target_not_eligible` | Selected target disappeared or is not a valid result destination. |
| `reconciliation_unsupported` (new) | A divergent independently added UUID collision or a chosen result requiring unsupported lifecycle/source behavior; details name the affected locator, reason and any supported alternatives. A divergent identity collision has no replacement alternatives. |
...
Use fresh separate weak agents to prepare parallel proposals from one product-created R0, including independent and same-result work, differing Component text, differing exact Relationship multiplicity, and composition conflicts. Submit useful feedback on B, accept A to R1, then give a fresh weak agent the business resolution task with normal CLI skill or MCP discovery only. It must discover reconciliation, use both side choices and a manual typed result, preserve displaced children explicitly where exercised, apply, inspect, review and deliberately accept B to R2.

A stronger independent verifier checks exact refs/objects, new base/generation, residual reconstruction/tree equality, unchanged proposal identity/Markdown, old Review parent/anchor reachability, Applied receipts, unchanged Accepted during reconciliation, and full process restart. No merge commit, extra refs, history chain, registry or alternate candidate is permitted. The real human checks a small actual reconciliation interaction, resulting proposal and exact historical feedback, rather than repeating the agents' authoring work.

## 9. Decisions and deliberate exclusions

The formerly blocking existing-child reassignment is an approved ordinary domain capability, with its exact operational schema/API/UX defined here. No additional domain extension is currently needed for a first slice with the explicitly choice-scoped unsupported lifecycle boundary and unsupported divergent new-UUID collisions. Complex interactions remain unresolved until a valid explicit assignment exists; feasibility is not guessed.

The operational version-2 encoding, command/tool/error names and concrete resolution contract are approved. A worker finding a supported but unrepresentable result must return it, not extend the closed model silently.

Exclude Component/Diagram deletion or restore, identity/source restoration, automatic anchors, free-floating Diagrams, multiple parents/children per anchor, generic hierarchy/graph/merge-operation infrastructure, raw YAML/Git patches, merge commits, persisted reconciliation sessions, conflict history, reconciliation refs, proposal history, automatic/background reconciliation, merge-and-accept, review verdict policy/comment migration/resolution, Relationship/appearance IDs, SQLite/worktrees/registries, remote collaboration/permissions, other verticals, and rich-diagram Phase 3.

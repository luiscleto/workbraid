...
The approved [retained comparison](architecture-proposals-v0.md#read-only-version-comparisons) adds **Compare versions → Before / After → Report**. The workspace action uses the existing unsent-edit guard. Dedicated `/projects/:slug/compare` and `/compare/report` entries are read-only, independent of process project selection. Two aligned selectors, Swap versions and a styled Report action use the workspace paper/ink/control palette and loaded fonts. Group by readable Accepted history, open proposals, accepted proposals and submitted reviews; explain that external merge ancestors need not have been individually accepted in WorkBraid. Submitted reviews first choose an existing review-owned proposal, including discarded ones. Page choices explicitly; duplicate names receive minimal identity context. Labels use proposal name plus Base/Proposed version; lifecycle/generation/exact identities are in details. A new comparison defaults Before to current exact Accepted history and After to the originating proposal candidate or submitted-review candidate when known, otherwise empty. Explicit URL choices, including partial, stale or unavailable choices, always take precedence. Never silently replace a retained choice, including after Swap, return navigation or reload. Stale, invalid and unavailable sources explain the next action without exposing backend errors as normal prose.

The report URL contains the store and both complete exact selectors. Show Before/After provenance and any exact side-labelled proposal documents, then reuse the concise report body: one After-primary highlighted image per affected Diagram, side-owned geometry and truthful Before-only absence. Presentation and raw diff remain opt-in. Equal versions say “No Architecture differences between these versions.” Existing proposal printing remains available. Print exact project/store and both sides' source, revision/tree and relevant proposal state/review identity as a compact human-labelled block kept together; selector JSON remains in machine results or collapsed screen details, never the default PDF. Actual wide/narrow controls, fonts, error states and PDF pagination require browser review; the final human gate remains separate.

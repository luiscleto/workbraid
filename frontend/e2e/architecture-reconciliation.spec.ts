...
test('placement reconciliation combines independent nodes and keeps old Diagram feedback exact', async ({app,page}) => {
  ...
  await expect(task).not.toContainText('Automatic')
...
test('ordinary browser Change parent component keeps the existing child', async ({ app, page }) => {
  ...
  await expect(summary.locator('.reassignment-review-label')).toHaveText([movement])
  await expect(summary.locator('.composition-review-change')).toHaveCount(1)
  await expect(summary.getByRole('button', { name: 'Comment on Before changes', exact: true })).toBeVisible()
  await expect(summary.getByRole('button', { name: 'Comment on With changes', exact: true })).toBeVisible()
...
test('one reassignment preserves separate Before and With feedback across parent contexts and restart', async ({ app, page }) => {
  await page.setViewportSize({ width: 1280, height: 900 })
  const setup = proposal(app, 'Initial parent contexts')
  const east = component(app, setup, 'East'); const west = component(app, setup, 'West')
  const left = detail(app, setup, east, 'Platform'); const right = detail(app, setup, west, 'Platform')
  const from = mutate(app, setup, ['component', 'create'], ['--diagram-id', left, '--title', 'Gateway']).component_id
  const to = mutate(app, setup, ['component', 'create'], ['--diagram-id', right, '--title', 'Gateway']).component_id
  const child = detail(app, setup, from, 'Operations'); accept(app, setup)
  const p = proposal(app, 'Move Operations')
  mutate(app, p, ['diagram', 'reassign-detail'], ['--diagram-id', child, '--anchor-component-id', to])
  mutate(app, p, ['component', 'edit'], ['--component-id', from, '--title', 'Gateway updated'])
  mutate(app, p, ['component', 'edit'], ['--component-id', to, '--title', 'Gateway updated'])
  mutate(app, p, ['diagram', 'edit-title'], ['--diagram-id', right, '--title', 'Platform updated'])
  mutate(app, p, ['diagram', 'edit-title'], ['--diagram-id', child, '--title', 'Operations updated'])
  const bound = app.cli(['change-set', 'review', ...state(app, p)]).result
  const original = inspect(app, p)
  const accepted = app.cli(['architecture', 'inspect']).context.accepted_revision
  expect(bound.comparison.appearances).toHaveLength(2)
  const beforeParent = bound.before.diagrams.find((item: any) => item.id === left)
  const withParent = bound.with_changes.diagrams.find((item: any) => item.id === right)
  const fromName = `Gateway — ${bound.before.components.find((item: any) => item.id === from).filename}`
  const toName = `Gateway updated — ${bound.with_changes.components.find((item: any) => item.id === to).filename}`
  expect(beforeParent.context).toBeTruthy()
  const movement = `Operations updated moved from ${fromName} to ${toName}`
  const beforeTarget = `Before changes · ${fromName} → Operations · in Platform — ${beforeParent.context}`
  const withTarget = `With changes · ${toName} → Operations updated · in Platform updated`
  const proposalURL = `${app.origin}/projects/${app.slug}/proposals/${p.id}`
  await page.goto(`${proposalURL}/review`)
  const summary = page.getByRole('region', { name: 'Diagram changes', exact: true })
  const move = summary.locator('.composition-review-change')
  await expect(move).toHaveCount(1)
  await expect(move.locator('.reassignment-review-label')).toHaveText(movement)
  await expect(summary.getByRole('button', { name: 'Platform updated title changed', exact: true })).toBeVisible()
  await expect(summary.getByRole('button', { name: 'Operations updated title changed', exact: true })).toBeVisible()
  const beforeButton = move.getByRole('button', { name: beforeTarget, exact: true })
  const withButton = move.getByRole('button', { name: withTarget, exact: true })
  const side = page.getByRole('group', { name: 'Review side', exact: true })
  await beforeButton.click()
  await expect(side.getByRole('button', { name: 'Before changes', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.diagram-breadcrumbs strong')).toHaveText('Platform')
  const beforeMovement = `Operations moved from ${fromName} to ${toName}`
  await expect(page.getByRole('region', { name: 'Review context', exact: true })).toContainText(beforeMovement)
  await expect(page.locator('.map-bottom-dock').getByRole('button', { name: `Composition: ${beforeMovement}`, exact: true })).toBeVisible()
  await expect(page.getByRole('region', { name: 'Review context', exact: true })).not.toContainText('Operations updated')
  await expect(page.locator(`[data-diff-path="${bound.comparison.appearances.find((item: any) => item.side === 'before').path}"]`)).toBeFocused()
  await move.getByRole('button', { name: 'Comment on Before changes', exact: true }).click()
  const beforeEditor = page.getByRole('region', { name: `Comment on ${beforeTarget}`, exact: true })
  await beforeEditor.getByRole('textbox', { name: 'Comment', exact: true }).fill('Before: this is the original parent link.\n')
  await withButton.click()
  await expect(page.getByRole('dialog', { name: 'Leave without keeping?' })).toBeVisible()
  await page.getByRole('button', { name: 'Keep editing', exact: true }).click()
  await expect(beforeEditor.getByRole('textbox', { name: 'Comment', exact: true })).toHaveValue('Before: this is the original parent link.\n')
  await move.getByRole('button', { name: 'Comment on With changes', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Leave without keeping?' })).toBeVisible()
  await page.getByRole('button', { name: 'Keep editing', exact: true }).click()
  await beforeEditor.getByRole('button', { name: 'Add comment', exact: true }).click()
  await withButton.click()
  await expect(side.getByRole('button', { name: 'With changes', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.diagram-breadcrumbs strong')).toHaveText(withParent.title)
  await expect(page.getByRole('region', { name: 'Review context', exact: true })).toContainText(movement)
  await expect(page.locator('.map-bottom-dock').getByRole('button', { name: `Composition: ${movement}`, exact: true })).toBeVisible()
  await expect(page.locator(`[data-diff-path="${bound.comparison.appearances.find((item: any) => item.side === 'with_changes').path}"]`)).toBeFocused()
  await move.getByRole('button', { name: 'Comment on With changes', exact: true }).click()
  const withEditor = page.getByRole('region', { name: `Comment on ${withTarget}`, exact: true })
  await withEditor.getByRole('textbox', { name: 'Comment', exact: true }).fill('With: this is the destination parent link.\n')
  await withEditor.getByRole('button', { name: 'Add comment', exact: true }).click()
  await move.scrollIntoViewIfNeeded()
  await screenshot(page, 'grouped-parent-move-two-comments')
  expect(inspect(app, p)).toEqual(original)
  await page.getByRole('textbox', { name: 'Reviewer name', exact: true }).fill('Parent reviewer')
  await page.getByRole('button', { name: 'Submit review', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Comment', exact: true })).toBeVisible()
  ...
  const feedback = app.cli(['review-submission', 'inspect', '--store-id', app.store, '--change-set-id', p.id, '--review-id', submissions[0].id]).result
  expect(feedback.reviewed_state).toBe(bound.reviewed_state)
  expect(feedback.comments.map((comment: any) => ({ body: comment.body, anchor: comment.anchor }))).toEqual([
    { body: 'Before: this is the original parent link.\n', anchor: { kind: 'composition', side: 'before', diagram_id: left, component_id: from, aspect: 'detail', detail_diagram_id: child } },
    { body: 'With: this is the destination parent link.\n', anchor: { kind: 'composition', side: 'with_changes', diagram_id: right, component_id: to, aspect: 'detail', detail_diagram_id: child } },
  ])
  expect(inspect(app, p)).toEqual(original)
  mutate(app, p, ['change-set', 'rename'], ['--name', 'Later proposal work'])
  await app.restart()
  await page.reload()
  await expect(page.getByText('Feedback on an earlier proposal version.')).toBeVisible()
  await expect(move.locator('.reassignment-review-label')).toHaveText(movement)
  await expect(move.getByRole('button', { name: /Comment on (Before|With) changes/ })).toHaveCount(0)
  await move.getByRole('button', { name: `1 review comment on ${beforeTarget}`, exact: true }).click()
  await expect(side.getByRole('button', { name: 'Before changes', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByLabel('Open review comments')).toContainText('Before: this is the original parent link.')
  await move.getByRole('button', { name: `1 review comment on ${withTarget}`, exact: true }).click()
  await expect(side.getByRole('button', { name: 'With changes', exact: true })).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByLabel('Open review comments')).toContainText('With: this is the destination parent link.')
  await move.scrollIntoViewIfNeeded()
  await screenshot(page, 'grouped-parent-move-historical')
  const retained = app.cli(['review-submission', 'inspect', '--store-id', app.store, '--change-set-id', p.id, '--review-id', feedback.id]).result
  expect(retained.reviewed_state).toBe(feedback.reviewed_state)
  expect(retained.comments).toEqual(feedback.comments)
  expect(retained.review).toEqual(feedback.review)
  expect(app.cli(['architecture', 'inspect']).context.accepted_revision).toBe(accepted)
  writeFileSync(join(evidence, 'grouped-parent-move-feedback.json'), JSON.stringify({ original, feedback, retained }, null, 2))
  await page.getByRole('button', { name: 'Proposal workspace', exact: true }).click()
  await expect(page).toHaveURL(proposalURL)
  await expect(page.getByRole('heading', { name: 'Later proposal work', exact: true })).toBeVisible()
  await expect(page.getByText('Before: this is the original parent link.', { exact: true })).toHaveCount(0)

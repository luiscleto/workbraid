...
test('durable placement keeps ordinary and boundary peers stationary, auto-layout, review frame and restart',async({page},testInfo)=>{
  ...
  try {
    ...
    const peers=(except:string)=>map.evaluate((el,except)=>{const cy=(el as any)._cyreg.cy;return cy.nodes().filter((n:any)=>!n.data('uiAnnotation')&&n.id()!==except).map((n:any)=>({id:n.id(),position:{...n.position()}})).sort((a:any,b:any)=>a.id.localeCompare(b.id))},except)
    ...
    const ordinaryPeers=await peers(worker)
    ...
    expect(drops).toBe(0)
    expect(await peers(worker)).toEqual(ordinaryPeers);expect(await frame()).toEqual(before)
    ...
    expect(await peers(worker)).toEqual(ordinaryPeers)
    ...
    const boundaryKey=proposed.candidate.diagrams.find((d:any)=>d.id===root).boundaries.find((b:any)=>b.component_id===external).key
    const boundaryStart=await node(boundaryKey),boundaryPeers=await peers(boundaryKey),boundaryFrame=await frame()
    expect(boundaryStart.grab).toBe(true)
    await page.mouse.move(boundaryStart.x,boundaryStart.y);await page.mouse.down();await page.mouse.move(boundaryStart.x+45,boundaryStart.y+30,{steps:8})
    expect(drops).toBe(2);expect(await peers(boundaryKey)).toEqual(boundaryPeers);expect(await frame()).toEqual(boundaryFrame)
    const boundaryDrop=page.waitForResponse(r=>r.url().endsWith('/diagrams/set-position'))
    await page.mouse.up();expect((await boundaryDrop).ok()).toBe(true)
    await expect.poll(()=>inspect(id).generation).toBe(3)
    await expect(page.locator('.component-documentation').getByRole('heading',{name:'External delivery',exact:true})).toBeVisible()
    expect(await peers(boundaryKey)).toEqual(boundaryPeers);expect(await frame()).toEqual(boundaryFrame)
    const boundaryPosition=(await node(boundaryKey)).model
    expect(inspect(id).candidate.diagrams.find((d:any)=>d.id===root).boundaries.find((b:any)=>b.component_id===external).position).toEqual(boundaryPosition)
    await expect(page.getByRole('button',{name:'Open home · Runtime',exact:true})).toBeVisible()
    await page.screenshot({path:join(runtimeRoot,'boundary-drag.png'),fullPage:true})
    await page.getByRole('button',{name:'Open home · Runtime',exact:true}).click()
    await page.getByRole('navigation',{name:'Diagrams and components'}).getByRole('button',{name:'Placement browser',exact:true}).click()
    expect((await node(boundaryKey)).model).toEqual(boundaryPosition)
    expect(await peers(boundaryKey)).toEqual(boundaryPeers)
    await page.getByRole('button',{name:'Back to proposal',exact:true}).click()
    const beforeLayout=inspect(id).candidate
    await page.getByRole('button',{name:'Auto-layout',exact:true}).click()
    await expect.poll(()=>inspect(id).generation).toBe(4)
    const afterLayout=inspect(id).candidate
    expect(afterLayout.diagrams.filter((d:any)=>d.id!==root)).toEqual(beforeLayout.diagrams.filter((d:any)=>d.id!==root))
    expect(afterLayout.components).toEqual(beforeLayout.components)
    const selectedLayout=afterLayout.diagrams.find((d:any)=>d.id===root)
    expect(selectedLayout.relationships).toEqual(beforeLayout.diagrams.find((d:any)=>d.id===root).relationships)
    expect(selectedLayout.appearances.every((a:any)=>a.position!==null)).toBe(true)
    const layoutState=inspect(id).change_set_state
    const unchangedLayout=page.waitForResponse(r=>r.url().endsWith('/diagrams/auto-layout'))
    await page.getByRole('button',{name:'Auto-layout',exact:true}).click();expect((await unchangedLayout).ok()).toBe(true)
    expect(inspect(id).change_set_state).toBe(layoutState)
    ...
    await page.getByRole('button',{name:'Keep position',exact:true}).click()
    await expect.poll(()=>inspect(id).generation).toBe(5)
    await page.getByRole('button',{name:'Back to proposal',exact:true}).click()
    await page.getByRole('button',{name:'Review changes',exact:true}).click()

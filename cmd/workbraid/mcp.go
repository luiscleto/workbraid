...
func registerMCPTools(server *mcp.Server, client *agentapi.Client) {
	...
	addMCPTool[agentapi.ChangeSetReviewRequest](server, client, "change_set_review", "Review change set", "Review one exact active generation against its original base and return the exact binding required by architecture_update plus review_url and printable_url. Printing reads the already prepared exact state without mutation; stale active links fail. Out-of-date work remains reviewable.", mutationAnnotations("Review change set", false, true))

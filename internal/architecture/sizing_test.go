...
func TestSizingClosedPortableAndOperationalSchemas(t *testing.T) {
	...
	for _, bad := range []string{strings.Replace(string(raw), "version: 4", "version: 3", 1), strings.Replace(string(raw), "node_sizes:", "other_sizes:", 1), strings.Replace(string(raw), "width: 200", "width: 200.5", 1)} {

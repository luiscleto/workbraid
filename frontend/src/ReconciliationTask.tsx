...
export function ReconciliationTask({ name, initial, onCheck, onApply, onLeave, onDirty, renderCandidate }: {
  ...
  function sideText(side: Side): string {
	...
	if(side.size&&(side.state==='stored'||side.state==='derived'))return `${side.state==='derived'?'Legacy displayed size · ':''}${side.size.width} × ${side.size.height}`
  ...
  function choiceLabel(conflict: Conflict, choice: ReconciliationResolution['choice']): string {
    ...
    if (choice === 'clear') return 'Clear this tuple’s routing'
    ...
    <div className="reconciliation-body">
      ...
      <div className="reconciliation-decision">
        {showResult && candidate ? <>
          ...
          {current.locator.kind==='route_loss'&&<><p>The final count or visibility cannot retain this routing. Clear acknowledges default routes for the whole tuple.</p>{current.route_loss?.map(loss=><p key={loss.side}>{loss.side==='accepted'?'Accepted':'Proposed'} · {loss.reason==='visibility_changed'?'Visibility changed':'Count changed'} · occurrences {loss.addresses.map(a=>a.occurrence).join(', ')}</p>)}<p>To preserve incompatible routing, return to the proposal and edit it before preparing again.</p></>}
          ...
          {!!current.choices.length && <section className="reconciliation-choice-section"><h4>{current.locator.reason === 'competing_children' ? 'Choose which diagram stays' : 'Choose the result'}</h4><div className="reconciliation-side-choices" role="group" aria-label="Choose result">{(['accepted', 'proposed', 'manual', 'clear'] as const).filter((choice) => current.choices.includes(choice)).map((choice) => <button type="button" key={choice} aria-pressed={resolution?.choice === choice} onClick={() => choose(current, choice, current.locator.kind === 'composition' ? choice === 'manual' ? resolution?.value ?? {} : choice==='clear'?undefined:sideValue(current, choice) : choice === 'manual' ? manualInitial(current) : undefined)}>{choiceLabel(current, choice)}</button>)}</div></section>}

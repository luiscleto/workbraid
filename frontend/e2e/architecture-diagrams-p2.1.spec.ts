...
test('P2.1 Diagram hierarchy remains navigable and writable on the living v2 product', async ({ page }) => {
  ...
  try {
    ...
    await expect(gatewayComposition.getByRole('button', { name: 'Change where it lives' })).toHaveClass(/text-action/)
    ...
    await expect(workerCompositionActions.getByRole('button', { name: 'Create detail diagram' })).toHaveClass(/text-action/)
    await expect(workerCompositionActions.getByRole('button', { name: 'Change where it lives' })).toHaveClass(/text-action/)

...
## Read-only retained version comparisons

Root approved proposal `0a03625f-40a0-4e17-84a8-dbb833091d2e`, generation 1, state `c9d389ea96a76389ff5c47246e80b2452c8c8255`, after independent review. This authorizes bounded comparison of two exact same-store retained snapshots. Accepted history discovery follows current Accepted ancestry, including external merge ancestors; reachability is retention evidence, not proof that WorkBraid individually accepted every ancestor. Validate that authority before using the existing version-aware historical loader. Other inputs resolve only through exact current active/applied records or immutable submitted-review parents. No arbitrary unretained Git object, new history/ref/database, report capture or authoring capability is added. Valid externally removed objects remain present or absent exactly on their own side. Reads neither repair supported history nor change process project selection. [Proposals](architecture-proposals-v0.md#read-only-version-comparisons) owns exact source lifetimes; Agent Access owns typed discovery. This supersedes prior history exclusions only for these retained read-only comparisons.

Phase 3.4 is authorized by delegated root approval of proposal `3cf11d4b-c980-4022-92d2-f98d75bf36a4`, generation 2, reviewed state `d9d35b11134533016248e5e64294462f76b3aeda`, independently approved in review `02d1d542-2f29-4f1a-a401-38ee66b10a3c`. Native initialization uses portable v6. This supersedes earlier native-version and shape/note exclusions; supported portable v2–v5 retain their exact schemas and source/history fidelity. [Placement §11](architecture-placement-amendment-v0.md#11-simple-shapes-and-diagram-notes-phase-34) owns the closed extension. Shapes are Diagram presentation addressed by Diagram/Component UUID. Plain notes are independent Diagram-owned values, with immutable Diagram-local UUIDs; they are never Components, Relationships, hierarchy anchors, reusable objects or submitted review comments. No catalog, runtime, Component/Relationship identity or acceptance authority changes.

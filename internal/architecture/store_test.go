...
func TestInitializeCreatesAndLoadsExactBootstrap(t *testing.T) {
	...
	if parsed.Format != "workbraid-architecture" || parsed.Version != 3 || parsed.StoreID != storeID || parsed.Project.Name != "Example Project" || parsed.Project.Slug != projectSlug || parsed.RootDiagram == "" {
	...
	if snapshot.FormatVersion() != 3 || snapshot.RootDiagramID() != parsed.RootDiagram {
...
func v2HomeDiagramTree(t *testing.T, storePath string, manifestBytes []byte, sources ...[]byte) string {
	t.Helper()
	parsed, err := parseManifest(manifestBytes)
	if err != nil || (parsed.Version != 2 && parsed.Version != 3) {

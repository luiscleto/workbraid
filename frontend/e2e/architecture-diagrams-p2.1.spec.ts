...
test('P2.1 Diagram hierarchy remains navigable and writable on the living v2 product', async ({ page }) => {
  ...
  try {
    ...
    const reviewBoundaryDock = page.locator('.diagram-boundary-dock')
    const reviewMapRegion = page.locator('.map-region')
    await expect(reviewControls).toBeVisible()
    await expect(reviewBoundaryDock).toBeVisible()
    let controlsBox = await reviewControls.boundingBox()
    let boundaryBox = await reviewBoundaryDock.boundingBox()
    let mapBox = await reviewMapRegion.boundingBox()
    expect(controlsBox).not.toBeNull()
    expect(boundaryBox).not.toBeNull()
    expect(mapBox).not.toBeNull()
    expect(boundaryBox!.y + boundaryBox!.height).toBeLessThanOrEqual(controlsBox!.y - 10)
    expect(Math.abs((mapBox!.y + mapBox!.height) - (boundaryBox!.y + boundaryBox!.height) - 176)).toBeLessThanOrEqual(1)

    await page.setViewportSize({ width: 720, height: 720 })
    await expect(reviewControls).toBeVisible()
    await expect(reviewBoundaryDock).toBeVisible()
    controlsBox = await reviewControls.boundingBox()
    boundaryBox = await reviewBoundaryDock.boundingBox()
    mapBox = await reviewMapRegion.boundingBox()
    expect(controlsBox).not.toBeNull()
    expect(boundaryBox).not.toBeNull()
    expect(mapBox).not.toBeNull()
    expect(boundaryBox!.y + boundaryBox!.height).toBeLessThanOrEqual(controlsBox!.y - 10)
    expect(Math.abs((mapBox!.y + mapBox!.height) - (boundaryBox!.y + boundaryBox!.height) - 156)).toBeLessThanOrEqual(1)
    await expect(page.getByRole('button', { name: 'Fit map' })).toBeVisible()

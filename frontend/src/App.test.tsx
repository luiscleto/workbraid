...
describe('App', () => {
  ...
  it('navigates one accepted-v2 Diagram projection coherently while keeping authoring available', async () => {
    ...
    expect(actionButtons[2]).toHaveClass('text-action', 'diagram-composition-link')
  ...
  it('keeps a candidate-only detail Diagram reachable through Changes in progress', async () => {
    ...
    expect(createDetail).toHaveClass('text-action', 'diagram-composition-link')
    expect(within(compositionActions).getByRole('button', { name: 'Change where it lives' })).toHaveClass('text-action', 'diagram-composition-link')
  ...
  it('guards a dirty home destination and keeps the move through the Diagram endpoint', async () => {
    ...
    const moveForm = destination.closest('form')
  ...
  it('keeps a structured Diagram title edit without exposing schema details', async () => {
    ...
    const titleForm = title.closest('form')
    expect(titleForm).toHaveClass('component-form', 'diagram-editor')
    expect(titleForm).not.toHaveClass('component-editor')
    expect(titleForm?.closest('.working-pane')).toBeInTheDocument()

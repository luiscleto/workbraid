...
test('P2.1 Diagram hierarchy remains navigable and writable on the living v2 product', async ({ page }) => {
  ...
  try {
    ...
    await expect(navigator.getByRole('button', { name: 'Nested helper' })).toHaveCount(0)
    ...
    await expect(navigator.getByRole('button', { name: 'Nested helper' })).toBeVisible()
    ...
    await expect(page.getByRole('navigation', { name: 'Diagrams and components' }).getByRole('button', { name: 'Nested helper' })).toBeVisible()

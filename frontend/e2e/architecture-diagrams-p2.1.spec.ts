...
test('P2.1 Diagram hierarchy remains navigable and writable on the living v2 product', async ({ page }) => {
  ...
  try {
    ...
    const diagramTitleAction = navigator.getByRole('button', { name: 'Edit title' })
    await expect(diagramTitleAction).toBeVisible()
    await expect(diagramTitleAction).toHaveClass(/index-add/)
    const componentPane = page.locator('.component-documentation')
    await expect(componentPane.getByRole('button', { name: 'Edit title' })).toHaveCount(0)
    await expect(componentPane.getByRole('button', { name: 'Edit component' })).toHaveClass(/inline-action/)
    await expect(componentPane.getByRole('button', { name: 'Open Detail' })).toHaveClass(/detail-link/)
    const gatewayComposition = componentPane.getByRole('group', { name: 'Diagram composition' })
    const moveHomeAction = gatewayComposition.getByRole('button', { name: 'Change where it lives' })
    await expect(moveHomeAction).toHaveClass(/diagram-composition-link/)
    await expect(moveHomeAction).toHaveCSS('border-bottom-style', 'solid')
    await moveHomeAction.focus()
    await expect(moveHomeAction).toBeFocused()
    await expect(moveHomeAction).toHaveCSS('outline-style', 'solid')
    await moveHomeAction.hover()
    await expect(moveHomeAction).toHaveCSS('color', 'rgb(36, 72, 47)')
    await expect(gatewayComposition.getByRole('button', { name: 'Create detail diagram' })).toHaveCount(0)
    ...
    await expect(page.getByText('Worker documentation A.')).toBeVisible()
    const workerCompositionActions = componentPane.getByRole('group', { name: 'Diagram composition' })
    await expect(workerCompositionActions.getByRole('button', { name: 'Create detail diagram' })).toHaveClass(/diagram-composition-link/)
    await expect(workerCompositionActions.getByRole('button', { name: 'Change where it lives' })).toHaveClass(/diagram-composition-link/)
    ...
    await expect(navigator.getByRole('button', { name: 'Edit title' })).toBeVisible()
    await expect(page.locator('.working-pane').getByRole('button', { name: 'Edit title' })).toHaveCount(0)
    ...
    await navigator.getByRole('button', { name: 'Worker' }).click()
    await page.getByRole('button', { name: 'Edit component' }).click()
    await page.getByLabel('Description').fill('Worker documentation A with nested composition.\n')
    await page.getByRole('button', { name: 'Keep change' }).click()
    ...
    await expect(pendingEditTitle).toHaveCSS('outline-style', 'solid')
    await expect(addNestedComponent).toHaveClass(/pending-diagram-action/)
    await addNestedComponent.click()
    await expect(page.getByRole('heading', { name: 'Add component' })).toBeVisible()
    await page.getByLabel('Title').fill('Nested helper')
    await page.getByLabel('Description').fill('Lives directly in the pending detail diagram.\n')
    await page.getByRole('button', { name: 'Keep change' }).click()
    await expect(workerInternals).toContainText('Nested helper')
    const nestedHelperComposition = workerInternals.getByText('Nested helper', { exact: true }).locator('..')
    await nestedHelperComposition.getByRole('button', { name: 'Create detail diagram' }).click()
    await page.getByLabel('Diagram title').fill('Nested helper internals')
    await page.getByRole('button', { name: 'Keep change' }).click()
    await expect(pendingComposition).toContainText('Nested helper internals')
    await expect(navigator.getByRole('button', { name: 'Nested helper', exact: true })).toHaveCount(0)
    await expect(navigator.getByRole('button', { name: /Worker internals/ })).toHaveCount(0)
    await workerComposition.getByRole('button', { name: 'Change where it lives' }).click()
    await expectContextualDiagramForm(page)
    const homeDestination = page.getByRole('combobox', { name: 'Diagram' })
    await expect(homeDestination.getByRole('option', { name: 'Worker internals' })).toHaveCount(0)
    await expect(homeDestination.getByRole('option', { name: 'Nested helper internals' })).toHaveCount(1)
    await homeDestination.selectOption({ label: 'Nested helper internals' })
    await page.getByRole('button', { name: 'Keep change' }).click()
    await expect(page.getByRole('alert')).toContainText('Move this component somewhere outside its own detail diagrams.')
    const compositionAttention = page.getByRole('region', { name: 'Diagram composition needs attention' })
    await expect(compositionAttention).toContainText('Worker')
    await expect(compositionAttention).toContainText('All other changes in progress are still kept.')
    await expect(page.locator('.pending-diagram-row')).toHaveCount(0)
    await expect(page.locator('.pending-home-move-row')).toHaveCount(0)
    await page.getByRole('button', { name: 'Review changes' }).click()
    await expect(compositionAttention).toContainText('Move this component somewhere outside its own detail diagrams.')
    await page.getByRole('button', { name: 'Fix component location' }).click()
    await page.getByRole('combobox', { name: 'Diagram' }).selectOption({ label: 'System A' })
    await page.getByRole('button', { name: 'Keep change' }).click()
    await expect(page.getByRole('region', { name: 'Diagram changes in progress' })).toContainText('Nested helper internals')
    await page.getByRole('button', { name: 'Review changes' }).click()
    await expect(page.getByRole('region', { name: 'Diagram changes' })).toContainText('Worker internals added')
    await expect(page.getByRole('region', { name: 'Diagram changes' })).toContainText('Worker placed in diagram')
    await expect(page.getByRole('region', { name: 'Diagram changes' })).toContainText('Nested helper placed in diagram')
    await page.getByRole('button', { name: 'Update architecture' }).click()
    const nestedRevision = await displayedRevision(page)
    expect(nestedRevision).not.toBe(firstRevision)
    await expect(navigator.getByRole('button', { name: /Worker internals/ })).toBeVisible()
    await navigator.getByRole('button', { name: /Worker internals/ }).click()
    await expect(navigator.getByRole('button', { name: 'Nested helper', exact: true })).toBeVisible()

    await stopWorkBraid(application)
    application = await startWorkBraid(binary, dataRoot, port, runtimeRoot, 'nested-reopen.log')
    await openProject(page, application.origin, sourceRoot)
    expect(await displayedRevision(page)).toBe(nestedRevision)
    await expect(page.getByRole('navigation', { name: 'Diagrams and components' }).getByRole('button', { name: /Worker internals/ })).toBeVisible()
    await page.getByRole('navigation', { name: 'Diagrams and components' }).getByRole('button', { name: /Worker internals/ }).click()
    await expect(page.getByRole('navigation', { name: 'Diagrams and components' }).getByRole('button', { name: 'Nested helper', exact: true })).toBeVisible()

    const secondRevision = writeAcceptedV2(storePath, bootstrapRevision, nestedRevision, storeID, sourceRoot, 'B')

...
Reuse exact existing Review projections/comparison and source. Render design Markdown first, affected Diagrams second, per-Diagram concrete changes and source-text diffs third. Visible Component source or Relationship multiset changes affect a Diagram even without a Diagram blob delta. Empty canonical diff means proposal text only and zero diagrams; a nonempty format-only canonical diff retains its complete technical appendix even with zero affected Diagrams. Component source diffs appear once at With home (Before home if absent), cross-linked elsewhere. LF boundaries, CRLF, whitespace and absent final newline remain distinguishable; safe source diffs make no rendered-Markdown semantic claim. Notes are Diagram text, not submitted comments. UI owns browser-memory images and browser Print / Save as PDF.

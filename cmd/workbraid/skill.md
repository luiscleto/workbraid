...
Read-only comparisons: `architecture versions --store-id <uuid> --source accepted|proposal|applied|review_proposals|submitted_review [--limit 25] [--cursor <returned>]` lists bounded retained sources without selecting the process current project. Maximum page size is 50 records (proposal/review records can contribute base and candidate). `review_proposals` returns only review-owned proposal IDs and names, including discarded proposals; `submitted_review` requires `--change-set-id <uuid>` to list that proposal's exact reviewed parent sides. An unavailable source is not selectable. Continue with the exact returned cursor; changed Accepted requires reloading choices, never silently replacing selected versions.

`architecture compare --store-id <uuid>` takes explicit `--before-*` and `--after-*` flags. Each side requires `--before-kind accepted --before-revision <exact commit>`, or `--before-kind proposal|applied --before-change-set-id <uuid> --before-state <exact current/receipt state> --before-side base|candidate`, or `--before-kind submitted_review --before-change-set-id <uuid> --before-review-id <uuid> --before-state <exact reviewed parent> --before-side base|candidate`. After uses the same fields prefixed `--after-`. Copy selectors from discovery; no arbitrary tree, ref expression or unretained object is supported. Accepted history includes validated same-store merge ancestors, not proof that each ancestor was individually accepted in WorkBraid. Active selectors expire on any state movement/application/discard; applied/submitted selectors follow existing retention. Accepted ancestry selectors require continued reachability from current Accepted.

MCP `architecture_versions` uses matching snake_case page fields; `architecture_compare` takes `store_id`, closed typed `before` and `after` selectors (`kind`, plus `revision` or `change_set_id/state/side` and `review_id` for submitted reviews). Both use the same server and remain additive agent-v2 reads. Results include exact Before/After snapshots, changes/diff, side-labelled document context and `report_url`. Valid unprepared candidates need no Review prep. Reports create no capture, ref, retention or acceptance binding and never update Architecture. Two Accepted versions have no invented proposal text. Open report URLs for explicit browser Print / Save as PDF; presentation/raw diff remain opt-in. `version_moved` means choose that side again; `version_unavailable` means inspect choices without repair or raw Git fallback.

Bound Review and proposal inspection return `printable_url`. Open it for a read-only document and explicit browser Print / Save as PDF. Active `/print?reviewed_state=S` requires the current already prepared state; applied `/print?applied_state=T` requires its exact receipt state. Opening/printing never prepares Review or mutates refs. Old active links fail after edits/application. `review-submission inspect` returns `/reviews/<review-id>/print`, retaining its exact parent after iteration/application/discard. These links have no arbitrary Git-history fallback. Print design Markdown first, then affected Diagrams, then concrete changes and exact source-text diffs; Markdown-only proposals show no diagrams. No print mutation or PDF service exists.

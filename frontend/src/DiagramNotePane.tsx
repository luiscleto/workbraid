...
export function DiagramNotePane({note,readOnly,busy,onDirty,onKeep,onDelete,onClear}:{note?:DiagramNote;readOnly:boolean;busy:boolean;onDirty:(v:boolean)=>void;onKeep:(v:Omit<DiagramNote,'id'>)=>Promise<boolean>;onDelete:()=>Promise<boolean>;onClear:()=>void}) {
 ...
 {readOnly?<><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{text}</pre>{note&&<details className="position-controls"><summary>Position and size</summary><dl>{(['x','y','width','height'] as const).map(k=><div key={k}><dt>{k[0].toUpperCase()+k.slice(1)}</dt><dd>{note[k]}</dd></div>)}</dl></details>}</>:<form onSubmit={async e=>{e.preventDefault();if(!text.trim()||Array.from(text).length>2000)return;const values={text,x:Number(geometry.x),y:Number(geometry.y),width:Number(geometry.width),height:Number(geometry.height)};if(await onKeep(values))onDirty(false)}}>
 <label>Text<textarea aria-label="Note text" rows={8} required disabled={busy} value={text} onChange={e=>{setText(e.target.value);onDirty(true)}}/></label><p className="field-hint">Plain text · up to 2000 characters</p>
 {note&&<details className="position-controls"><summary>Position and size</summary><div className="position-fields">{(['x','y','width','height'] as const).map(k=><label key={k}>{k[0].toUpperCase()+k.slice(1)}<input aria-label={`Note ${k}`} type="number" step={1} required disabled={busy} min={k==='width'?120:k==='height'?48:-100000} max={k==='width'?800:k==='height'?600:100000} value={geometry[k]} onChange={e=>{setGeometry({...geometry,[k]:e.target.value});onDirty(true)}}/></label>)}</div></details>}

...
- Human or explicitly delegated root approval identifies the exact proposal UUID and generation/text. Submitted verdicts remain informational and alone grant no implementation permission. Change canonical meaning only after the authorized decision; Architecture acceptance remains separate.
- Planning discovery: store `7329b076-50c4-4ac2-b63d-cb5cdb2a87fa`; inspect exact catalog/proposal records. [Current proposal links](docs/roadmap.md) identify delegated drafts. Markdown-only plans remain active, not Applied; no new acceptance lifecycle.
